...
//                Intent intent = new Intent(requireContext(), MainActivity.class);
//
//                // 🔴 මේක අලුතෙන් දැම්මා (විශේෂ පණිවිඩය)
//                intent.putExtra("FROM_ORDER_COMPLETE", true);
//
...
public class OrderCompleteFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        binding.btnMyOrder.setOnClickListener(v -> {
            ...
            // අපේ අතින්ම Order Fragment එක ලෝඩ් කරනවා
            requireActivity().getSupportFragmentManager().beginTransaction()
                    .replace(R.id.fragmentContainer, new OrderFragment())
                    .commitAllowingStateLoss();

            // Bottom Nav එකේ Order අයිකන් එක නිල් පාට කරනවා
        ...
        // 🟢 2. ෆෝන් එකේ Back Button එක එබුවම වෙන දේ
        requireActivity().getOnBackPressedDispatcher().addCallback(getViewLifecycleOwner(), new androidx.activity.OnBackPressedCallback(true) {
            @Override
            public void handleOnBackPressed() {
                // හැංගිලා තියෙන Bottom Nav එක ආයෙත් ගන්නවා
                ...
                // අපේ අතින්ම Home Fragment එක ලෝඩ් කරනවා (Bottom Nav එකට බාර දෙන්නේ නෑ)
                requireActivity().getSupportFragmentManager().beginTransaction()
                        .replace(R.id.fragmentContainer, new HomeFragment())
                        .commitAllowingStateLoss();

                // Bottom Nav එකේ Home අයිකන් එක නිල් පාට කරනවා
                if (bottomNavView instanceof com.google.android.material.bottomnavigation.BottomNavigationView) {
                    ...
                    try {
                        if (documentSnapshot.exists()) {
                            ...
                            if (order != null) {
                                ...
                                // 🔴 Check for multiple items and calculate max time
                                if (items != null && items.size() > 1) {
    ...
    // 🔴 New method to find max preparation time for multiple items
    private void getMultipleItemsTime(List<Order.OrderItem> items) {
        ...
        // Firebase 'whereIn' supports up to 10 queries
